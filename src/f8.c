...
uint8_t R[64]; // 64 byte Scratchpad

uint8_t A   = 0; // Accumulator
uint16_t PC0 = 0; // Program Counter
uint16_t PC1 = 0; // Program Counter alternate
uint16_t DC0 = 0; // Data Counter
uint16_t DC1 = 0; // Data Counter alternate
uint8_t ISAR = 0; // Indirect Scratchpad Address Register (6-bit)
...
{
	return (R[reg&0x3F]<<8) | R[(reg+1)&0x3F];
}
